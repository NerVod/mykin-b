...
exports.accepterAmi = async (req, res) => {
  ...
  try {
    const _Accepteur = await User.findOne({ email: accepteur }, (err, user) => {
      if (!err) {
        const _UserInvited = [userInvited];
        const invitAttenteAccepteur = user.invitEnAttente;
        const listeinvitEnAttenteApresAccept = invitAttenteAccepteur.filter(
          (obj) => _UserInvited.indexOf(obj) === -1
        );

        // vérif si le nouvel ami a été demandé de mon coté en ami
        let invitEnvoyee = user.invitEnvoyee;
        const invitEnvoyeeApresAccept = invitEnvoyee.filter(
          (obj) => _UserInvited.indexOf(obj) === -1
        );
        let Amis = user.amis;
    ...
    const _Inviteur = await User.findOne(
      ...
      (err, user) => {
        if (!err) {
          ...
          let invitEnvoyeeInviteurApresAccept = invitEnvoyee.filter(
            (obj) => inviteurAClean.indexOf(obj) === -1
          );

          // invit en attente à nettoyer
          const invitEnAttente = user.invitEnAttente;

          let invitEnAttenteApresAccept = invitEnAttente.filter(
            (obj) => inviteurAClean.indexOf(obj) === -1
          );
          let Amis = user.amis;
          Amis.push(accepteur);
...
exports.getAllFriends = async (req, res) => {
  ...
  try {
    User.findOne({ email: activeUser }, (err, user) => {
      if (!err) {
    ...
    const supprimeur = await User.findOne(
      ...
      (err, user) => {
        if (!err) {
          ...
          User.findOne({ email: userDeleted }, (err, user) => {
            if (!err) {
...
exports.getInscrits = async (req, res) => {
  try {
    const totalInscrits = await User.find();
    // console.log('total inscrits nombre ', totalInscrits.length)
    res.json({ nombreinscrits: totalInscrits.length });
  } catch {
    console.log("erreur de recherche d'inscrits");
  }
};
